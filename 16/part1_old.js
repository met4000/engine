(() => {
  ...
  traverse({ x: -1, y: 0 }, { dx: 1, dy: 0 });

  // return hasBeam.map(v => v.map(v => v[HORIZONTAL] || v[VERTICAL] ? "#" : ".").join("")).join("\n");
  // return hasBeam.map((v, y) => v.map((v, x) => MIRROR.concat(SPLITTER).includes(grid[y][x]) ? grid[y][x] : v[HORIZONTAL] || v[VERTICAL] ? "#" : ".").join("")).join("\n");
  return hasBeam.reduce((r, v) => r + v.reduce((r, v) => r + (v[HORIZONTAL] || v[VERTICAL]), 0), 0)
})();
