...
const HAND_SCORING = "J23456789TQKA";
...
// remove jokers from sorted hand, and count
.replace(/^/gm, "0:")
.repeatReplace(v => v
  .replace(/^(\d+:[^,]*)J([^,]*,)/gm, "2+$1$2")
  .r_add_sub()
)
...
.repeatReplace(v => v
  .replaceTemplate(
    templateRegExp`/^(\d+:[^,]*)(${i => HAND_SCORING[i]})([${i => HAND_SCORING.slice(i + 1)}])([^,]*),/gm`,
    template`$1$3$2$4,`,
    range(0, HAND_SCORING.length - 2)
  )
)

// score the hands
.replace(/^(\d+:)(?:([^|,])\2{4}),/gm, "10|$1")
.replace(/^(\d+:)(?:[^|,]{0,1}([^|,])\2{3}[^|,]{0,1}),/gm, "8|$1")
.replace(/^(\d+:)(?:([^|,])\2{2}([^|,])\3|([^|,])\4([^|,])\5{2}),/gm, "7|$1")
.replace(/^(\d+:)(?:[^|,]{0,2}([^|,])\2{2}[^|,]{0,2}),/gm, "6|$1")
.replace(/^(\d+:)(?:[^|,]?([^|,])\2[^|,]?([^|,])\3[^|,]?),/gm, "5|$1")
.replace(/^(\d+:)(?:[^|,]{0,3}([^|,])\2{1}[^|,]{0,3}),/gm, "4|$1")
.replace(/^(\d+:)(?:[^|,]+),/gm, "2|$1")
.replace(/^(\d+:),/gm, "0|$1")

// add score from jokers
.replace(/\|/g, "+")
.r_add_sub()
...
.repeatReplace(v => v
  .replaceTemplate(
    templateRegExp`/^(\d*)([${n => n + 1}-9])(\d*)(\|+)([^|]+)\n\1(${n => n})(\d*)\4([^|]+)$/gm`,
    "$1$6$7$4$8\n$1$2$3$4$5",
    range(1, 8)
  )
)

// sort by hand contents
.repeatReplace(v => v
  .replaceTemplate(
    templateRegExp`/^(\d+\|+[${HAND_SCORING}]*)([${i => HAND_SCORING.slice(i + 1)}])([${HAND_SCORING}]*,.+)\n\1(${i => HAND_SCORING[i]})([${HAND_SCORING}]*,.+)$/gm`,
    "$1$4$5\n$1$2$3",
    range(0, HAND_SCORING.length - 2)
  )
)

// replace with just bids
.replace(/^.+,(\d+)$/gm, "$1")
