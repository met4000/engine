...
const HAND_SCORING = "23456789TJQKA";
...
.repeatReplace(v => v
  .replaceTemplate(
    templateRegExp`/^([^,]*)(${i => HAND_SCORING[i]})([${i => HAND_SCORING.slice(i + 1)}])([^,]*),/gm`,
    template`$1$3$2$4,`,
...
// score the hands
.replace(/^([^|,])\1{4},/gm, "10||")
.replace(/^[^|,]{0,1}([^|,])\1{3}[^|,]{0,1},/gm, "8|")
.replace(/^([^|,])\1{2}([^|,])\2,|^([^|,])\3([^|,])\4{2},/gm, "7|")
.replace(/^[^|,]{0,2}([^|,])\1{2}[^|,]{0,2},/gm, "6|")
.replace(/^[^|,]?([^|,])\1[^|,]?([^|,])\2[^|,]?,/gm, "5|")
.replace(/^[^|,]{0,3}([^|,])\1{1}[^|,]{0,3},/gm, "4|")
.replace(/^[^|,]+,/gm, "2|")
.replace(/^,/gm, "0|")
