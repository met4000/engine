...
// a for left, w for up, d for right
.replace(templateRegExp`/([7|F])(.{${INPUT_WIDTH}})S/s`(), "$1%$2w") // up
.replace(/([F\-L])S/, "$1&a") // left
.replace(/S([J\-7])/, "d$1*") // right
...
// replace the starting tile with the correct tile
.replace(/a%/, "1").replace(/a\^/, "j").replace(/a&/, "_")
.replace(/w\*/, "j").replace(/w%/, "|").replace(/w&/, "l")
.replace(/d\*/, "_").replace(/d%/, "f").replace(/d\^/, "l")

// wipe every tile not on the path
.replace(/[^f1jl'_.\n]/g, ".")


// determine outside/inside tiles by sweeping through
...
.repeatReplace(v => v
  ...
  // don't invert if the left and right sides of the line point the same way
  .replace(/(!|\|)(f_*1|l_*j)/, "$2$1")

  // invert if at a vertical pipe or if the side of the line point opposite ways
  .replace(/\|('|f_*j|l_*1)/, "$1!")
  .replace(/!('|f_*j|l_*1)/, "$1|")
